...
**Failure modes:** Missing metadata (now 500, no silent fall-through), webhook delay, double clicks (mitigated by idempotency + locks), manual SQL without updating `current_participants`. **Stripe retry / concurrent deliveries** can both pass “no existing row” and race on `INSERT` → Postgres **`23505`** unique on `(session_id, youth_wrestler_id)`; webhook must treat that as success (update paid flags, do not double-increment count).
