...
## 5. Legacy early adopter entitlements (DB only)

**Current product:** Discount codes at signup / Account grant **percent-off** (`parent_percentage_discounts`). **Book-a-coach** does not read `early_adopter_entitlements` for pricing or “free session” messaging.

**Legacy:** The `early_adopter_entitlements` table and admin “Grant early adopter” may still exist for old rows. Do not assume parents with rows get automatic $0 private booking in the app unless a specific legacy checkout path still sets Stripe metadata (`early_adopter_entitlement_id`). Prefer documenting real flows in code (signup, redeem, bookings API, register API).
...
- **Discount codes:** Percent-off on sessions via signup or Account redeem (see `docs/EARLY_ADOPTER_CODES.md`). Legacy entitlement rows (§5) are separate.
