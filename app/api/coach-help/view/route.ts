...
/** POST — log a view (embed loaded or user opened the video). Body: { videoKey } */
export async function POST(req: NextRequest) {
  try {
    ...
    const { error } = await supabase.from('coach_help_views').insert({
      user_id: user.id,
      video_key: videoKey,
    });

    if (error) {
      console.error('coach_help_views insert:', error.message);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error('coach-help view POST:', e);
